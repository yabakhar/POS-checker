...
export default function ClientLogin() {
  ...
  useEffect(() => {
    ...
    if (token && role === 'client') navigate('/dashboard', { replace: true });
  }, [navigate]);
  ...
  const inpStyle = (name: string): React.CSSProperties => ({
    ...
    border: `1px solid ${focused === name ? T.accentClient : T.border}`,
  ...
  const gradientBg = dark
    ? `radial-gradient(ellipse 80% 50% at 50% -5%, #0A1E35 0%, ${T.bg} 65%)`
    : `radial-gradient(ellipse 80% 50% at 50% -5%, #BAE6FD 0%, ${T.bg} 65%)`;
  ...
  return (
    <div style={{ minHeight: '100vh', background: gradientBg, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }}>
      ...
      <button onClick={toggle}
        ...
        onMouseEnter={(e) => { e.currentTarget.style.borderColor = T.accentClient; e.currentTarget.style.color = T.accentClient; }}
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 32 }}>
          ...
          <div>
            ...
            <div style={{ fontSize: 10, color: T.muted, fontWeight: 600, letterSpacing: 1, textTransform: 'uppercase' }}>Portail client</div>
          </div>
        </div>

        <h1 style={{ fontSize: 22, fontWeight: 700, letterSpacing: -0.4, marginBottom: 6, color: T.text }}>Connectez-vous</h1>
        <p style={{ fontSize: 14, color: T.muted, marginBottom: 28, lineHeight: 1.5 }}>Accédez à votre tableau de bord POS</p>
        ...
        <form onSubmit={handleSubmit}>
            ...
            <div style={{ position: 'relative' }}>
              ...
              <button type="button" onClick={() => setShowPass((v) => !v)}
                style={{
                  ...
                  color: showPass ? T.accentClient : T.subtle,
                  display: 'flex', alignItems: 'center', padding: 2, transition: 'color 150ms',
                }}
                onMouseEnter={(e) => (e.currentTarget.style.color = T.muted)}
                onMouseLeave={(e) => (e.currentTarget.style.color = showPass ? T.accentClient : T.subtle)}
              >
          ...
          <button type="submit" disabled={loading}
            style={{
              ...
              background: T.accentClient, color: '#fff',
        ...
        <p style={{ textAlign: 'center', marginTop: 24, fontSize: 13, color: T.muted }}>
          Admin ?{' '}
          <a href="/admin/login"
            style={{ color: T.accent, fontWeight: 500, transition: 'opacity 150ms' }}
